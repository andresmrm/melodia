...
class Melodia():
    ...
    def tocar2(self, lista_freq, tempo):
        ...
        for freq in lista_freq:
            for i in range(tempo * self.frequencia):
                valor = math.sin(2 * math.pi * (
                    float(i) / float(self.frequencia)) * float(freq))
    ...
    def gerar_onda(self, freq, tempo_nota):
        # return ondas.gerar_onda(
        #     float(freq), float(tempo_nota), float(self.frequencia))
        frames = []
        ampli = [1.0]
    ...
    try:
        ...
        while resp is not "s":
            ...
            if resp is "n":
                ...
                m.tocart(m.ler_arq("musicas/teste"), 10)
    #            m.ler_arq("ode")
